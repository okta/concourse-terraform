...
function buildImage() {
    ...
    pushd "${DOCKER_REPOSITORY_PATH}"
        docker build --build-arg RELEASE_VERSION="$RELEASE_VERSION"  --build-arg "TERRAFORM_VERSION=${TERRAFORM_VERSION}"  -f ${DOCKER_FILE_PATH} ${DOCKER_REPOSITORY_NAME}':'${RELEASE_VERSION}.
    popd
}
